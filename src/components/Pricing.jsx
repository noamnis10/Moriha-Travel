  ...
  return (
    <section id="pricing" className="bg-paper py-24">
      <div className="mx-auto max-w-6xl px-6">
        ...
        {query?.destination && (
          <div className="mt-5 flex items-center gap-3 rounded-full bg-paper-soft px-4 py-2 text-sm text-ink-soft w-fit">
            ...
            <button onClick={() => setQuery(null)} className="flex items-center gap-1 font-semibold text-teal-700 hover:underline">
              <X className="h-3.5 w-3.5" /> ניקוי סינון
            </button>
          </div>
        )}

        <div ref={reveal.ref} className={`mt-10 grid grid-cols-1 gap-7 md:grid-cols-2 lg:grid-cols-3 ${reveal.className}`}>
          {filtered.map((pkg) => {
            const isRecommended = pkg.id === cheapestId;
            return (
              <article
                key={pkg.id}
                className={`flex flex-col overflow-hidden rounded-3xl border bg-paper shadow-sm transition hover:-translate-y-1.5 hover:shadow-lg ${
              ...
              >
                ...
                <div className="flex flex-1 flex-col gap-4 p-6">
                  <div className="flex items-center justify-between">
                    ...
                    {isRecommended && (
                      <span className="rounded-full bg-teal-900 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-white">
                        המומלץ שלנו
                      </span>
                    )}
                  </div>
